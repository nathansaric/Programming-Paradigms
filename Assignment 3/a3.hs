-- Nathan Saric - 03/14/2022

module A3
where
import Data.List  -- for using lookup function in Q2b
import Data.Maybe -- for using fromJust function in Q2b
...
getValuations []       = [[]]
getValuations (c : cs) = (map (\xs -> (c, True ) : xs) (getValuations cs) ++ 
                          map (\xs -> (c, False) : xs) (getValuations cs))
...
evalF valu (Atom c)            = (fromJust(lookup c valu))
    ...
    in
      case focus of
        ...
        Or phi1 phi2 ->        -- Or-Left rule
            prove' (done ++ [phi1, phi2] ++ rest) goal 
            (\() -> prove' (done ++ [phi2] ++ rest) goal (kSucceed, kFail), 
             \() -> prove' (done ++ [phi1] ++ rest) goal (kSucceed, kFail))
...
test_or5 = not (prove [Or vA vB] (And vC vD))
